...
// AsyncLocalStorage is used to store the traceId in the async context
...
const customFormat = winston.format.combine(
    ...
    winston.format.printf((info) => {
        ...
        return `${info.timestamp} [${traceId}] ${info.level.toUpperCase()} - ${info.message} `
    }),
    winston.format.colorize({ all: false })
)

let info = {
    filename: `log/info/14-ims-api/14-ims-api__%DATE%.log`,
...
let debug = {
    filename: `log/debug/14-ims-api/14-ims-api__%DATE%.log`,
    datePattern: "YYYY-MM-DD",
    zippedArchive: true,
    level: "debug",
    format: customFormat
...
module.exports.setTraceId = function(traceId) {
...
module.exports.getTraceId = function() {
    return asyncLocalStorage.getStore()?.traceId || 'NO_TRACE_ID'
}
